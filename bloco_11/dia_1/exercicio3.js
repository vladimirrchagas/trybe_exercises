// HTML

<div id="root"></div>

// CSS

body {
  margin: 1em;
}

.card-img-top {
  width: 30%;
  heigth: auto;
}

// JS

function Card(props) {
  return (
    <div className="card">
      <img className="card-img-top" src={props.featureImage} alt={props.title} />
      <div className="card-body">
        <h5 className="card-title">{props.title}</h5>
        <p className="card-text">{props.description}</p>
        <a href={props.link} className="btn btn-primary">Learn more</a>
      </div>
    </div>
  );
}

function CardList() {
return (
  <div className="row">
    <div className="col-sm-4">
      <Card
        featureImage="https://sebhastian.com/img/nathan-sebhastian.png"
        title="How To Make Interactive ReactJS Form"
        description="Let's write some interactive form with React"
        link="https://sebhastian.com/interactive-react-form"
      />
    </div>
    <div className="col-sm-4">
      <Card
        featureImage="https://sebhastian.com/img/nathan-sebhastian.png"
        title="Babelify your JavaScript code"
        description="Babel make JavaScript code browser-compatible."
        link="https://sebhastian.com/babel-guide"
      />
    </div>
    <div className="col-sm-4">
      <Card
        featureImage="https://sebhastian.com/img/nathan-sebhastian.png"
        title="JavaScript Basics Before You Learn React"
        description="Learn the prerequisites of learning React fast"
        link="https://sebhastian.com/js-before-react"
      />
    </div>
    
{/*Start changes*/}
    
    <div className="col-sm-4">
      <Card
        featureImage="https://sebhastian.com/img/nathan-sebhastian.png"
        title="Importing CSS in React"
        description="Learn how to include CSS inside React applications"
        link="https://sebhastian.com/react-import-css/"
      />
    </div>
    
    <div className="col-sm-4">
      <Card
        featureImage="https://sebhastian.com/img/nathan-sebhastian.png"
        title="React Props vs State"
        description="Let's learn the difference between Props and State in React"
        link="https://sebhastian.com/react-props-vs-state/"
      />
    </div>
    
{/*Finish changes*/}
    
  </div>
);
}

ReactDOM.render(<CardList />, document.getElementById('root'))
