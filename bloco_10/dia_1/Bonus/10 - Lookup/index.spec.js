...
const lookup = (login, property) => {
  ...
  const result = users.filter(element => element.login === login);

  if(result.length === 0) throw new Error('Could not find user');

  if(!result[0][property]) throw new Error('Could not find property');
